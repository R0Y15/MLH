...
def scaner(data, extractor, definitions, matcher=None):
...
def _simple_matcher(regex, data):
    regex = deJSON(regex)
    match = re.search(regex, data)
    return match.group(1) if match else None


def _replacement_matcher(regex, data):
    try:
        regex = deJSON(regex)
...
def checker(results, definitions):
    for r in results:
        result = r
...
def pos_finder(version1, version2):
