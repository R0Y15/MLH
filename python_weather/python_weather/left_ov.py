...
from .cache_collector import Cache
from .responder import Weather
from .exceptioner import HTTPException
from python_weather import responder
...
class HTTPClient:
    ...
    async def req(self, location: str) -> dict:
        ...
        if error_message:
            raise HTTPException(responder, error_message['#text'])
        elif resp.status >= 400:
            raise HTTPException(responder)
